...
function CursorSwitcher(props: CursorSwitcherProps) {
    ...
    const buttonClassName = customCursor
        ? 'w-[44px] h-[44px] rounded-lg border flex items-center justify-center transition-all cursor-none border-[var(--border2)] text-[var(--text2)] hover:border-[var(--accent)] hover:text-[var(--accent)] hover:bg-[var(--accentbg)]'
        : 'w-[44px] h-[44px] rounded-lg border flex items-center justify-center transition-all cursor-none border-[var(--accent)] bg-[var(--accentbg)] text-[var(--accent)]'
    ...
    return (
        <button
            ...
            aria-label={customCursor ? 'Wyłącz własny kursor' : 'Włącz własny kursor'}
            type="button"
        >
            {customCursor
