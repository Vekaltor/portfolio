      ...
      <div className="max-w-[1240px] mx-auto px-16 w-full">
        <div className="grid gap-20 items-start" style={{ gridTemplateColumns: '1fr 1.8fr' }}>
          ...
          <div ref={tlWrapRef} className="relative mt-[2.8rem]">
            {/* Track line */}
            <div className="absolute left-[8px] top-[8px] bottom-0 w-[2px] rounded-[1px]"
              style={{ background: 'var(--border)' }}>
              <div ref={fillRef} className="tl-fill absolute top-0 left-0 w-full h-0 rounded-[1px]"
                style={{ background: 'linear-gradient(to bottom, var(--accent), var(--accentd))' }} />
            </div>

            {TIMELINE_ITEMS.map((item, i) => {
              const dateLabel = item.date.includes('tl.now')
                ? `11/2023 — ${t('tl.now')}`
                : item.date
              ...
              return (
                <div key={item.id} className="flex gap-[1.6rem] pb-[2.8rem] relative">
                  ...
                  />
                  <div ref={el => { ctxRefs.current[i] = el }} className="tl-content-hidden">
                    ...
                    <div className="flex flex-wrap gap-[.4rem] mt-[.9rem]">
                      {item.tags.map(tag => (
                        <span key={tag}
                          ...
                          style={{ background: 'var(--accentbg)', border: '1px solid rgba(74,222,128,.2)' }}>
                          {tag}
                        </span>
                      ))}
                    </div>
